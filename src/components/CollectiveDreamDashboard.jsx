...
const UI_COPY = {
  en: {
    ...
    loadStates: {
      ...
      fallback: "Public archive unavailable",
      empty: "No public records yet",
    },
    loadError: "The public archive could not be loaded. Recording and account tools remain available.",
  ...
  zh: {
    ...
    loadStates: {
      loading: "連線至公開檔案庫",
      live: "公開研究檔案庫",
      fallback: "公開檔案庫暫時無法載入",
      empty: "尚無公開紀錄",
    },
    loadError: "公開檔案庫暫時無法載入；你仍可記錄夢境或進入帳戶。",
    schemaFocus: "心理模式焦點",
    anomalySearch: "情緒標記",
  ...
  es: {
    ...
    loadStates: {
      ...
      fallback: "Archivo público no disponible",
      empty: "Aún no hay registros públicos",
    },
    loadError:
      "El archivo público no se pudo cargar. El registro y la cuenta siguen disponibles.",
...
function normalizeDreamCard(row) {
  ...
  return {
    ...
    titleEn:
      row.titleEn || row.title_en || row.publicTitleEn || publicVersionEn.title || "",
    title_en: row.title_en || row.titleEn || "",
    titleZh:
      row.titleZh || row.title_zh || row.publicTitleZh || publicVersionZh.title || "",
    title_zh: row.title_zh || row.titleZh,
    titleEs:
      row.titleEs || row.title_es || row.publicTitleEs || publicVersionEs.title || "",
